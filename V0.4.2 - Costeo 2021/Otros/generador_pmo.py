...
def generar_reporte(txt_seteos):
    ...
    resultado_exportacion_pmo = variables['resultado_exportacion_pmo']
    resultado_exportacion_no_pmo = variables['resultado_exportacion_no_pmo']
    ...
    dfc_pmo = dfc[dfc["Marca PMO"]=='NO PMO']
    dfc_no_pmo = dfc[dfc["Marca PMO"]=='PMO']
    
    td1 = pd.pivot_table(dfc_pmo, index = segmentacion, aggfunc = metricas, fill_value = 0)
    ...
    td11 = pd.pivot_table(dfc_no_pmo, index = segmentacion, aggfunc = metricas, fill_value = 0)
    td11.columns = col_td1


    td2 = funciones.td_consumo(df = dfc_pmo, columna = "Apertura Total", segmentacion = segmentacion, metricas = metricas)
    td12 = funciones.td_consumo(df = dfc_no_pmo, columna = "Apertura Total", segmentacion = segmentacion, metricas = metricas)
    
    print("fin td2 -- " + datetime.now().strftime('%H:%M:%S'))

    td3 = funciones.td_consumo(df = dfc_pmo, columna = "Amb por Rubros", segmentacion = segmentacion, metricas = metricas)
    td13 = funciones.td_consumo(df = dfc_no_pmo, columna = "Amb por Rubros", segmentacion = segmentacion, metricas = metricas)
    print("fin td3 -- " + datetime.now().strftime('%H:%M:%S'))

    td4 = funciones.td_consumo(df = dfc_pmo, columna = "Medicamentos por Origen", segmentacion = segmentacion, metricas = metricas)
    td14 = funciones.td_consumo(df = dfc_no_pmo, columna = "Medicamentos por Origen", segmentacion = segmentacion, metricas = metricas)
    print("fin td4 -- " + datetime.now().strftime('%H:%M:%S'))

    td5= funciones.td_consumo(df = dfc_pmo, columna = "Medicamentos por Via", segmentacion = segmentacion, metricas = metricas)
    td15= funciones.td_consumo(df = dfc_no_pmo, columna = "Medicamentos por Via", segmentacion = segmentacion, metricas = metricas)
    print("fin td5 -- " + datetime.now().strftime('%H:%M:%S'))

    td6 = funciones.td_consumo(df = dfc_pmo, columna = "Medicamentos por Cobertura", segmentacion = segmentacion, metricas = metricas)
    td16 = funciones.td_consumo(df = dfc_no_pmo, columna = "Medicamentos por Cobertura", segmentacion = segmentacion, metricas = metricas)
    print("fin td6 -- " + datetime.now().strftime('%H:%M:%S'))

    td7 = funciones.td_consumo(df = dfc_pmo, columna = "Tipo Int. ID", segmentacion = segmentacion, metricas = metricas)
    td17 = funciones.td_consumo(df = dfc_no_pmo, columna = "Tipo Int. ID", segmentacion = segmentacion, metricas = metricas)    
    print("fin td7 -- " + datetime.now().strftime('%H:%M:%S'))

    td8 = funciones.td_consumo(df = dfc_pmo, columna = "Marca PMO", segmentacion = segmentacion, metricas = metricas)
    td18 = funciones.td_consumo(df = dfc_no_pmo, columna = "Marca PMO", segmentacion = segmentacion, metricas = metricas)
    ...
    # td resumida con las prestaciones pmo
    tdt_pmo = pd.concat([td1, td2, td3, td4, td5, td6, td7, td8], axis=1)
    # td resumida sin las prestaciones pmo
    tddt_no_pmo = pd.concat([td11, td12, td13, td14, td15, td16, td17, td18], axis=1)
    ...
    # EXPORTAMOS EL DF PMO

    dff_pmo = pd.merge(left = dfs, right = tdt, 
                left_on=("Persona","Periodo"), right_on=("Persona","Periodo_Prest"), how = "left")

    dff_pmo = dff_pmo.fillna(0)
    dff_pmo["Prestaciones_PMO"] = "PMO"

    dff_pmo.to_csv(resultado_exportacion_pmo, encoding = 'latin1', index = False, decimal=',')


   # EXPORTAMOS EL DF NO PMO

    dff_no_pmo = pd.merge(left = dfs, right = tdt, 
                left_on=("Persona","Periodo"), right_on=("Persona","Periodo_Prest"), how = "left")

    dff_no_pmo = dff_pmo.fillna(0)
    dff_no_pmo["Prestaciones_PMO"] = "No PMO"

    dff_no_pmo.to_csv(resultado_exportacion_no_pmo, encoding = 'latin1', index = False, decimal=',')
